...
void main() {
    ...
    float sigmaAdapted = sigma * mouse.y * 0.001; // Scale the sigma value for the theta function

    vec2 complexValue;
    
    // // Split the screen: LHS renders Zeta function, RHS renders Theta function
    // if (vUv.x < 0.5) {
    //     // Left-hand side (LHS) for Zeta function
    //     complexValue = zeta(sigmaAdapted, tAdapted);
    // } else {
    //     // Right-hand side (RHS) for Theta Lattice function
    //     complexValue = thetaLattice(sigmaAdapted, tAdapted);
    // }
    ...
    vec3 gradient2 = mix(color3, gradient1, 0.0 + sin(0.5 * normalizedMagnitude * 0.5 * normalizedPhase));
